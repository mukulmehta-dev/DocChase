  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900">Recurring Templates</h1>
            ...
            <Card key={tpl.id} elevation="low" className="p-5 flex flex-col justify-between">
              <div>
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="font-semibold text-base text-slate-900">{tpl.name}</h3>
                ...
                <div className="mt-4 flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-1.5 max-h-48 overflow-y-auto">
                    {(tpl.items || []).map((item, idx) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between p-2 rounded bg-slate-50 border border-slate-200 text-xs text-slate-700"
                      >
              ...
              <div className="mt-5 pt-3.5 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                ...
                <span className="text-[11px] text-slate-400">Immutable snapshot on request</span>
              </div>
            </Card>
          ))}
        </div>
      ...
      >
        <form onSubmit={handleCreateTemplate} className="flex flex-col gap-4">
          ...
          <div>
            <label className="text-xs font-semibold text-slate-700 block mb-1">Cadence / Frequency</label>
            <select
              value={frequency}
              onChange={(e) => setFrequency(e.target.value as any)}
              className="w-full h-10 px-3 rounded-lg border border-slate-300 text-xs bg-white text-slate-800 focus:border-primary-container outline-none"
            >
          ...
          <div className="flex flex-col gap-2 mt-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-700">Document Requirement Slots</span>
              <button
                type="button"
            ...
            {items.map((it, idx) => (
              <div key={idx} className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 h-9 px-3 rounded-lg border border-slate-300 text-xs"
                />
