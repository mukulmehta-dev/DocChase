/**
 * Browser E2E Test: Auth Error UX & Password Rejection Verification
 * 
 * Target: http://localhost:4173/sign-in (Production build via vite preview)
 * 
 * Verifies:
...
async function run() {
  console.log('═════════════════════════════════════════════════════════');
  console.log('  DocChase Browser Test: Auth Error UX Verification');
  console.log('  Target: http://localhost:4173/sign-in');
  console.log('═════════════════════════════════════════════════════════\n');
  ...
  try {
    ...
    console.log('✅ Local preview server is healthy at http://localhost:4173/sign-in');
  } catch (e: any) {
    console.error('❌ Preview server not responding at http://localhost:4173/sign-in:', e.message);
    process.exit(1);
  }

  // Launch headless Chrome pointing directly to sign-in page
  const profileDir = path.resolve('scratch/chrome-test-error-profile');
  ...
  if (!pageTarget || !pageTarget.webSocketDebuggerUrl) {
    console.error('❌ Failed to connect to headless Chrome CDP on port 9223');
    chromeProcess.kill();
    process.exit(1);
  }
  ...
  ws.onmessage = (event) => {
    ...
    if (data.method === 'Network.responseReceived') {
      ...
      if (res.status === 400 && res.url.includes('grant_type=password')) {
        ...
        console.log('  [NET] Captured expected HTTP 400 from Supabase Auth:', res.url.split('?')[0]);
      }
    }
  };
  ...
  try {
    ...
    console.log('\n--- Step 1: Waiting for /sign-in to hydrate ---');
    await delay(3000);
    ...
    // Enter valid email + deliberately wrong password
    ...
    const fillRes = await send('Runtime.evaluate', {
      expression: `(() => {
        ...
        return 'Populated valid email + wrong password cleanly';
      })()`,
    });
    console.log('Input fill status:', fillRes?.result?.value);

    // Submit sign-in form with wrong password
    ...
    console.log('Waiting for authentication rejection & error UX rendering...');
    await delay(3500);

    // Assertions for error handling
    console.log('\n--- Step 4: Verifying Auth Error UX Assertions ---');

    // Assertion 1: Must stay on /sign-in
    ...
    console.log('URL after auth rejection:', currentHref);
    if (!currentHref.includes('/sign-in')) {
      throw new Error(`❌ Browser did not remain on /sign-in! Actual URL: ${currentHref}`);
    }
    console.log('  ✅ [PASS] Browser remains on /sign-in (no navigation to /dashboard or reload)');
    ...
    console.log('Waiting for login transition to /dashboard...');
    ...
    console.log('\n═════════════════════════════════════════════════════════');
    console.log('  🎉 All Browser Auth Error UX & Success Verifications PASSED!');
    console.log('═════════════════════════════════════════════════════════\n');
